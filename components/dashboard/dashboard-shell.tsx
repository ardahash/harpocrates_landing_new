...
const DEMO_MODE_KEY = "harpocrates-demo-mode"
...
export function DashboardShell() {
  ...
  useEffect(() => {
    if (isProd || initialized.current) return
      ...
      <div className="container mx-auto max-w-6xl space-y-10">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {!isProd && (
              <div className="flex items-center gap-2 rounded-full border border-border/60 bg-muted/40 px-3 py-1">
                ...
                <span className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Dev</span>
              </div>
            )}
        ...
        {demoMode && (
          <section className="rounded-xl border border-primary/30 bg-primary/5 p-5">
            <h2 className="text-lg font-semibold">Reviewer Demo Mode Active</h2>
            <p className="text-sm text-muted-foreground">
              Sample data is preloaded and the privacy demo auto-runs to showcase encrypted prompts, simulated enclave
              processing, and local decryption.
            </p>
          </section>
        )}
